...
- 按下停止翻译后要等很久才实际停止：停止操作此前阻塞在译音排空上
  （可能长达十几秒），期间界面停在"翻译中"。现在停止立即完成（停止
  麦克风、关闭会话、界面回到就绪），已排队的译音在**后台**按已知的
  播放时长播完后再释放播放器——既不卡界面，尾部也不再被截断。
- 同声传译停止后仍只播放一部分译音：排空改为依据计划播放结束时刻
  （`_scheduledPlaybackEndMicros`）等待，不再依赖轮询原生指标，避免
  链上 enqueue 被提前丢弃导致尾部丢失。重新开始翻译或回放会立刻接管
  播放器（旧尾部被切除）。

### Tests

- 130 Flutter tests pass; Android host tests and lint pass。
