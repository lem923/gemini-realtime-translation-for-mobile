...
class RestSentenceTranslator implements SentenceTranslator {
  ...
  /// REST fallback ASR for regions/keys where the flash live preview is
  /// rejected: gemini-2.5-pro accepts inline audio via generateContent.
  static Future<String> transcribeViaRest({
    required String apiKey,
    ...
    try {
      ...
      final String raw = await response.transform(utf8.decoder).join();
      if (response.statusCode != 200) {
        throw HttpException('ASR REST ${response.statusCode}');
      }
      final String text = _extractText(raw).trim();
      if (text.isEmpty) {
        throw const SentencePipelineException('语音识别没有返回结果');
      ...
      throw const SentencePipelineException('语音识别超时');
    } finally {
      client.close(force: true);
    }
  }
  ...
  static Uint8List _pcmToWav(Uint8List pcm) {
    ...
    header.setUint32(0, 0x52494646, Endian.little);
    header.setUint32(4, 36 + pcm.length, Endian.little);
    header.setUint32(8, 0x57415645, Endian.little);
    header.setUint32(12, 0x666d7420, Endian.little);
    ...
    header.setUint32(36, 0x64617461, Endian.little);
